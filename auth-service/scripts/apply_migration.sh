...
# Default values
DATABASE_DSN="postgres://postgres:postgres@0.0.0.0:5432/database?sslmode=disable"
...
command -v sqlfluff >/dev/null 2>&1 || {
  echo "❌ 'sqlfluff' not found. Install via pip or package manager." >&2
  exit 1
}

echo "🔍 Checking SQL files with sqlfluff..."
find ./migrations -name '*.sql' ! -name 'seed_*' \
  | xargs sqlfluff lint --dialect mysql
