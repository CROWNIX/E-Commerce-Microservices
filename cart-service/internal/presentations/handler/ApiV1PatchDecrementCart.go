...
func (h *Handler) ApiV1PatchDecrementCart(c *gin.Context) {
	var userID uint64
	...
	err = h.Service.CartService.DecrementCart(c.Request.Context(), userID, productID)
	if err != nil {
		ginx.ErrorResponse(c, err)
		return
	}

	resp := api.ApiV1PatchDecrementCartResponseBody{
		Message: "Decrement cart Successfully",
	}
