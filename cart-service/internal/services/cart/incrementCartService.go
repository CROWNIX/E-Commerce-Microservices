...
func (s *cartService) IncrementCart(ctx context.Context, userID uint64, productID uint64) (err error) {
	...
	if product.MaximumPurchase != nil && *product.MaximumPurchase == quantity {
		return apperror.BadRequest("Cannot increment cart. Reached maximum purchase limit")
	}

	err = s.cartRepositoryWriter.IncrementCart(ctx, userID, productID)
	if err != nil {
		return err
	}
